      ...
      <div className="w-full border-2 sm:border-4 border-zinc-800 bg-zinc-900 p-3 sm:p-5 flex flex-wrap items-center justify-between gap-3 sm:gap-4">
        <div className="flex items-center gap-3 sm:gap-6 flex-wrap w-full">
          ...
          <div className="flex flex-col gap-1.5 flex-1 min-w-0">
            <span className="text-[9px] sm:text-[10px] uppercase font-mono text-zinc-500 font-extrabold tracking-widest">Group Filter</span>
          ...
          <div className="flex flex-col gap-1.5 shrink-0">
            <span className="text-[9px] sm:text-[10px] uppercase font-mono text-zinc-500 font-extrabold tracking-widest">Status</span>
      ...
      <div className="space-y-6 sm:space-y-8">
        {filteredMatches.length === 0 ? (
          <div className="bg-zinc-900 border-2 sm:border-4 border-zinc-800 p-8 sm:p-12 text-center rounded-none">
            ...
            <span className="text-white font-display text-xl sm:text-2xl uppercase tracking-wider block">No Matches Found</span>
            <span className="text-[10px] sm:text-xs text-zinc-500 mt-1 block uppercase font-mono tracking-widest">Adjust filters to reveal matchups</span>
          </div>
        ) : (
          matchesByDate.map(({ date, matches: dateMatches }) => (
            <div key={date} className="space-y-3 sm:space-y-4">
              ...
              <div className="flex items-center gap-2 sm:gap-3 py-2">
                ...
                <span className="font-display text-[11px] sm:text-sm uppercase tracking-widest text-yellow-400 font-black truncate">{formatDate(date)}</span>
                <div className="flex-1 h-px bg-zinc-800" />
                <span className="text-[9px] sm:text-[10px] font-mono text-zinc-600 uppercase tracking-widest font-bold shrink-0">{dateMatches.length} {dateMatches.length === 1 ? "match" : "matches"}</span>
              </div>
          ...
          {dateMatches.map(match => {
            ...
            return (
              ...
              >
                {/* Match Header Info */}
                <div className="px-3 sm:px-5 py-2 sm:py-3 border-b-2 border-zinc-800 bg-black flex items-center justify-between text-[10px] sm:text-[11px] text-zinc-500 font-mono font-bold gap-2">
                  <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                    <span className="px-1.5 sm:px-2 py-0.5 bg-zinc-800 text-yellow-400 border border-zinc-700 text-[9px] sm:text-[10px] uppercase font-black shrink-0">
                      {match.group}
                    </span>
                    <span className="flex items-center gap-1 uppercase tracking-wider truncate">
                      <Calendar className="h-3 w-3 shrink-0" /> {match.date} • {match.time}
                    </span>
                  </div>
                  <span className="hidden sm:flex items-center gap-1 uppercase tracking-wider text-zinc-500 shrink-0">
                ...
                <div className="p-3 sm:p-6 flex flex-col items-center justify-between gap-4 sm:gap-6 relative bg-gradient-to-b from-zinc-900 to-zinc-950">
                  <div className="flex items-center justify-center gap-3 sm:gap-6 md:gap-12 w-full">
                    ...
                    <div className="flex flex-col items-center gap-1 sm:gap-3 sm:flex-row sm:w-44 text-center sm:text-left min-w-0">
                      <span className="text-3xl sm:text-5xl filter drop-shadow select-none shrink-0">{match.teamA.flag}</span>
                      <div className="min-w-0">
                        <span className="font-display text-lg sm:text-2xl uppercase tracking-tight text-white block truncate leading-none">{match.teamA.name}</span>
                        <span className="text-[9px] sm:text-[10px] text-zinc-500 font-mono uppercase tracking-widest block mt-0.5">{match.teamA.code}</span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-center gap-2 sm:gap-3 shrink-0">
                      {match.actualScore ? (
                        <div className="flex items-center gap-2 sm:gap-3">
                          <span className="text-3xl sm:text-4xl md:text-5xl font-display text-white tracking-widest leading-none">
                            {match.actualScore.teamA}
                          </span>
                          <span className="text-yellow-400 font-extrabold text-xl sm:text-2xl">-</span>
                          <span className="text-3xl sm:text-4xl md:text-5xl font-display text-white tracking-widest leading-none">
                            {match.actualScore.teamB}
                          </span>
                          {match.isLive && (
                            <span className="ml-1 sm:ml-2 px-1.5 sm:px-2 py-0.5 bg-red-600 text-white text-[9px] sm:text-[10px] font-black uppercase tracking-wider animate-pulse">
                              Live
                            </span>
                          )}
                        </div>
                    ...
                    <div className="flex flex-col items-center gap-1 sm:gap-3 sm:flex-row-reverse sm:w-44 text-center sm:text-right min-w-0">
                      <span className="text-3xl sm:text-5xl filter drop-shadow select-none shrink-0">{match.teamB.flag}</span>
                      <div className="min-w-0">
                        <span className="font-display text-lg sm:text-2xl uppercase tracking-tight text-white block truncate leading-none">{match.teamB.name}</span>
                        <span className="text-[9px] sm:text-[10px] text-zinc-500 font-mono uppercase tracking-widest block mt-0.5">{match.teamB.code}</span>
                      </div>
                    </div>
                  </div>
                ...
                {!isExpanded && (
                  <div className="px-3 sm:px-5 py-2 sm:py-3 bg-zinc-950 border-t-2 border-zinc-800 overflow-x-auto">
                    <div className="flex flex-wrap items-center gap-y-1.5 gap-x-2 sm:gap-x-4 text-[10px] sm:text-[11px] text-zinc-400">
                      <span className="font-mono text-[8px] sm:text-[9px] uppercase font-black tracking-widest text-yellow-400 shrink-0">AI:</span>
                      <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
                        {models.map(model => {
                          ...
                          return (
                            ...
                            >
                              <span className="text-zinc-500 font-sans mr-1 text-[9px] sm:text-[10px] uppercase font-bold">{model.name}:</span>
                              <span className="font-extrabold text-white text-[11px] sm:text-[12px]">{pred.teamAScore}-{pred.teamBScore}</span>
                            </div>
                          );
                        })}
                      </div>
                                ...
                                >
                                  <div>
                                    <div className="flex items-center gap-2 mb-2">
                                      <div className={`w-2 sm:w-2.5 h-2 sm:h-2.5 bg-gradient-to-r ${model.avatarColor}`}></div>
                                      <span className="font-black text-[11px] sm:text-xs text-white uppercase block truncate tracking-wider">{model.name}</span>
                                    </div>

                                    {/* Predicted Score Bubble */}
                                    <div className="text-center bg-black border border-zinc-800 p-2 font-mono mb-3">
                                      <div className="text-[8px] sm:text-[9px] text-zinc-500 uppercase tracking-widest font-bold">Predicted</div>
                                  ...
                                  {evalStats && (
                                    <div className={`text-center py-1.5 sm:py-2 px-2 text-[9px] sm:text-[10px] uppercase font-mono font-black border-2 ${evalStats.bg}`}>
                                      {evalStats.label}
                                    </div>
                                  )}
                                </div>
